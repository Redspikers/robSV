...
void Motor::back(int distanceMilliMeter) {
	...
	acceleration(105);
	while (((this->countPulseLeft / 20.25) * 189.028)
			< (distanceMilliMeter - 150)) {
		delay(5);
	}
...
void Motor::distanceTotaleParcourue(int countPulseRight, int countPulseLeft,
		int distanceTotaleRight, int distanceTotaleLeft) {
...
int Motor::distanceParcourue(int countPulseLeft, int distanceLeft) {
	...
	distanceLeft = ((countPulseLeft - this->nbPulseLeft_temp) / 20.25)
			* 189.028; //distance en mm
